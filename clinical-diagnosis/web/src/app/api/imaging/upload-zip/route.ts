...
async function forwardToCtModule(dicomFiles: { name: string; data: Buffer }[]) {
  ...
  try {
    const upstream = await fetch(`${CT_API}/ct-module/dicom/upload`, {
      method: "POST",
      body: uploadForm,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!file || !(file instanceof Blob)) {
      return NextResponse.json({ detail: "请上传 .zip 或 .dcm 文件" }, { status: 400 });
    }

    const filename = file instanceof File ? file.name : "upload";
    const lower = filename.toLowerCase();

    if (isDicomName(filename)) {
      const bytes = Buffer.from(await file.arrayBuffer());
      if (bytes.length === 0) {
        return NextResponse.json({ detail: "DICOM 文件为空" }, { status: 400 });
      }
      return forwardToCtModule([{ name: filename, data: bytes }]);
    }

    if (!lower.endsWith(".zip")) {
      return NextResponse.json({ detail: "请上传 .zip 压缩包或 .dcm 文件" }, { status: 400 });
    }
    ...
    if (dicomFiles.length === 0) {
      return NextResponse.json({ detail: "ZIP 中未找到有效 DICOM 文件" }, { status: 400 });
    }

    return forwardToCtModule(dicomFiles);
  } catch (err) {
